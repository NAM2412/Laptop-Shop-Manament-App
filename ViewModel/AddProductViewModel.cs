...
namespace QuanLyCuaHangDienTu.ViewModel
{
    public class AddProductViewModel : BaseViewModel
    {
        ...
        public AddProductViewModel()
        {
            ...
            {
                var sp = DataProvider.Ins.DB.SANPHAM.Where(x => x.MASP == MASP).Count();
                if (MASP == null || MASP == "" || TENSP == null || TENSP == "" || GIAGOCSP == null || GIAGOCSP == "" || GIABANSP == null || GIABANSP == "") 
                    return false;
                else if (sp > 0)
                {
                    return false;
        ...
        private void KiemTraMa()
        {
            ...
            if (MASP == null || MASP == "")
            {
                lbMASP = "Vui lòng nhập mã sản phẩm*";
            }
            else if (sp > 0)
            {
                lbMASP = "Mã sản phẩm đã tồn tại*";
            }
            else
            {
                lbMASP = "";
